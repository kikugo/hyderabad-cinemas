...
const TheaterExplorer = () => {
  ...
  const locations = useMemo(() => [...new Set(theaters.map(t => t.location))].sort(), [theaters]);
  const theaterTypes = useMemo(() => [...new Set(theaters.map(t => t.type))], [theaters]);
  ...
  const stats = useMemo(() => {
    ...
    const boutique = activeTheaters.filter(
      t => t.type === 'Boutique' || t.type === 'Twin Theaters'
    ).length;
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <header className="text-center mb-10">
            ...
            <p className="text-slate-400 text-lg max-w-2xl mx-auto">
              Discover {stats.total} movie theaters across the city of pearls — from iconic single
              screens to premium multiplexes
            </p>
          </header>
          ...
          <section className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-4 mb-8">
            ...
            <StatCard
              icon={LayoutGrid}
              label="Multiplexes"
              value={stats.multiplexes}
              color="from-purple-600 to-purple-700"
            />
            <StatCard
              icon={Film}
              label="Single Screens"
              value={stats.singleScreens}
              color="from-indigo-600 to-indigo-700"
            />
            <StatCard
              icon={Zap}
              label="Dolby Atmos"
              value={stats.atmosCount}
              color="from-pink-600 to-pink-700"
            />
            <StatCard
              icon={TrendingUp}
              label="4K Projection"
              value={stats.fourKCount}
            ...
            {mainView === 'theaters' && (
              <div className="flex items-center gap-3">
                <span className="text-slate-400 text-sm">
                  <span className="text-cyan-400 font-semibold">{filteredTheaters.length}</span>{' '}
                  theaters found
                </span>
          ...
          <main>
            {mainView === 'map' ? (
              <MapView theaters={filteredTheaters} onSelectTheater={setSelectedTheater} />
            ) : (
              <>
                {/* Theater Grid/List */}
                <div
                ...
                {mainView === 'favorites' && favorites.length === 0 && (
                  <div className="text-center py-20">
                    ...
                    <p className="text-slate-400">
                      Click the heart icon on theaters to add them to your favorites
                    </p>
                  </div>
                )}
          ...
          <footer className="mt-16 text-center text-slate-500 text-sm">
            <p>
              Data includes {stats.total} active theaters and {stats.upcoming} upcoming venues
            </p>
